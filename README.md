...
This repository is the NixOS configuration for a single workstation named `loca`. It is structured as a flake-based system definition with a shared module layer, a host profile, and a Home Manager user profile.
...
- `home/esaiaswestberg`: Home Manager configuration for the default user
- `secrets`: encrypted secrets and the bootstrap notes for them

## How the configuration is assembled

`flake.nix` defines the `loca` NixOS system and passes the host name and user name into the module graph. The host entrypoint in `hosts/loca/default.nix` imports the shared modules, the host-specific hardware files, and the Home Manager configuration.
...
- `home/esaiaswestberg` contains user-facing packages and terminal configuration that should follow the user rather than the machine
...
`home/esaiaswestberg/default.nix` wires in three Home Manager modules:
